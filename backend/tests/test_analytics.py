...
class TestKPIAnalytics:
    ...
    def test_excel_export(self, client, auth_headers):
        ...
        enq = client.post('/api/v1/enquiries/', json={
            'customerName': 'Client 1',
            'contactInformation': 'c1@test.com',
            'productServiceRequired': 'Service 1'
        }, headers=headers)
        ...
        client.put(f'/api/v1/bids/{bid_id}/status', json={
            'status': 'Order Received', 'note': 'Won'
        }, headers=headers)

        export_res = client.get('/api/v1/analytics/export/csv', headers=headers)
...
class TestCalendarView:
    def test_calendar_view(self, client, auth_headers):
        ...
        cal_res = client.get('/api/v1/bids/calendar', headers=headers)
        ...
        filtered_res = client.get('/api/v1/bids/calendar?month=2026-05', headers=headers)
        assert filtered_res.status_code == 200
        filtered_events = filtered_res.get_json()
        assert any(e['enquiryId'] == enquiry_id for e in filtered_events)

        non_matching_res = client.get('/api/v1/bids/calendar?month=2026-06', headers=headers)
...
class TestGlobalSearch:
    def test_global_search(self, client, auth_headers):
        ...
        upload_res = client.post('/api/v1/documents/upload', data={
            'bidId': bid_res.get_json()['_id'],
            'file': (io.BytesIO(b'Acme Docs'), 'acme_spec.pdf')
        }, headers=headers)
        assert upload_res.status_code == 201

        search_res = client.get('/api/v1/search?q=acme', headers=headers)
        ...
        empty_res = client.get('/api/v1/search?q=nonexistentxyz123', headers=headers)
...
class TestSLATracking:
    def test_sla_check_and_report(self, client, auth_headers):
        ...
        check_res = client.post('/api/v1/admin/sla/check', headers=headers)
        assert check_res.status_code == 200
        check_data = check_res.get_json()
        assert check_data['breaches'] >= 1

        report_res = client.get('/api/v1/admin/sla/report', headers=headers)
...
class TestAdminModelStatus:
    def test_model_status_endpoint(self, client, auth_headers):
        ...
        status_res = client.get('/api/v1/admin/model-status', headers=headers)
